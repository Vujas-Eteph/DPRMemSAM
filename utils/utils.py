...
def compute_seq_perf(pred_masks_, gt, bounds, sequence_name):
    ...
    for i in range(len(gt)):
        gt_ = gt[i]
        if not isinstance(gt_, Special) and not gt_.is_empty():
            if not isinstance(gt_, Rectangle):
                gt[i] = Rectangle.convert(gt_)
                pred_masks[i] = Rectangle.convert(pred_masks[i])    
