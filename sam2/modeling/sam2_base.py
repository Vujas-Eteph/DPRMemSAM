...
class SAM2Base(torch.nn.Module):
    ...
    def _prepare_memory_conditioned_features(
        ...
        if not is_init_cond_frame:
            ...
            # Optionally attend the init frame (frame 0) ONLY via the long-term bank, not as a DRM
            # conditioning frame: keep it in selected_cond_outputs (so the RAM selection below still
            # excludes it — the init must not land in a recency RAM slot), but drop its conditioning
            # tokens/pointer here and let the long-term injection add it back at the same conditioning
            # temporal slot. `_cond_attn` = the cond frames actually attended as DRM.
            _excl_init = (getattr(self, "_lt_exclude_init_from_cond", False)
                          and 0 in selected_cond_outputs
                          and 0 in (getattr(self, "_longterm_frames", None) or []))
            _cond_attn = {fid: out for fid, out in selected_cond_outputs.items()
                          if not (_excl_init and fid == 0)}
            # There is no temporal positional encoding for the DRM part of the memory
            t_pos_and_prevs = [(0, out) for out in _cond_attn.values()]
            ...
            # Long-term medoid memory (additive 3rd bank; see CustomDAM4SAMTracker). The tracker
            # maintains a causal list of representative frame indices on self._longterm_frames
            # (online k-medoids over IoU/presence-eligible PAST frames). Resolve each requested
            # frame's stored output once here; it is attended below as extra conditioning-style
            # tokens (spatial memory + object pointer), ON TOP of the default DRM + RAM. Frames
            # already attended as DRM/RAM are skipped so none is weighted twice. Empty (a no-op,
            # so default tracking is byte-identical) whenever the long-term bank is disabled/unset.
            _ram_attended = {int(i) for i, out in zip(ram_frame_indices, outs_) if out is not None}
            lt_outs = []
            for _t in sorted({int(x) for x in (getattr(self, "_longterm_frames", None) or [])}):
                if _t in _cond_attn or _t in _ram_attended:   # _cond_attn (not selected_cond_outputs):
                    continue                                  # an init excluded from DRM is re-added here
                _o = output_dict["cond_frame_outputs"].get(
                    _t, output_dict["non_cond_frame_outputs"].get(_t, None))
                if _o is not None and _o.get("maskmem_features") is not None:
                    lt_outs.append((_t, _o))
            ...
            # Long-term memory spatial tokens — attended at the conditioning temporal slot (the same
            # maskmem_tpos_enc slot this frame's DRM cond frames use, total_num_maskmem - 1), so they
            # are position-agnostic like the rest of the conditioning memory and leave the RAM temporal
            # ordering untouched.
            for _t, _o in lt_outs:
                _feats = _o["maskmem_features"].cuda(non_blocking=True)
                to_cat_memory.append(_feats.flatten(2).permute(2, 0, 1))
                _enc = _o["maskmem_pos_enc"][-1].cuda().flatten(2).permute(2, 0, 1)
                to_cat_memory_pos_embed.append(_enc + self.maskmem_tpos_enc[total_num_maskmem - 1])
            ...
            if self.use_obj_ptrs_in_encoder:
                ...
                if not self.training and self.only_obj_ptrs_in_the_past_for_eval:
                    ptr_cond_outputs = {
                        t: out
                        for t, out in _cond_attn.items()
                        if (t >= frame_idx if track_in_reverse else t <= frame_idx)
                    }
                else:
                    ptr_cond_outputs = _cond_attn
                ...
                # Long-term memory object pointers (treated like conditioning-frame pointers; their
                # temporal pos is the distance to the current frame, as for the cond pointers above).
                for _t, _o in lt_outs:
                    if _o.get("n_pixels_pos", 1) >= 1:
                        pos_and_ptrs.append((abs(frame_idx - _t), _o["obj_ptr"]))
