...
def select_closest_cond_frames(frame_idx, cond_frame_outputs, max_cond_frame_num):
    ...
    # max(0, ...): the always-kept frames (closest-before + the pinned init) can already exceed
    # max_cond_frame_num when it is small (e.g. 1). Without the clamp, num_remain goes negative and
    # `list[:negative]` keeps all-but-last instead of none — flooding DRM and overflowing the 7-slot
    # temporal encoding. Clamping to 0 means "add no more" once the forced frames fill the budget.
    num_remain = max(0, max_cond_frame_num - len(selected_outputs))
    inds_remain = sorted(
